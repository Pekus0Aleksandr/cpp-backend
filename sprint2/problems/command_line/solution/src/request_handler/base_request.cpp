...
namespace http_handler {
    ...
    StringResponse BaseRequestHandler::MakeStringResponse(http::status status, std::string_view responseText, unsigned http_version,
                                                          bool keep_alive, std::string_view content_type, bool no_cache) {
    ...
    StringResponse BaseRequestHandler::MakeBadResponse(http::status status, unsigned http_version,
    ...
    StringResponse BaseRequestHandler::MakeInvalidMethod(std::string_view allow_methods, unsigned http_version, bool keep_alive) const {
        auto text = app::JsonMessage("invalidMethod"sv, "Invalid method"sv);
        auto resp = MakeStringResponse(http::status::method_not_allowed, text, http_version, keep_alive, ContentType::APP_JSON, true);
    ...
    StringResponse BaseRequestHandler::MakeInvalidApiVersion(unsigned http_version, bool keep_alive) const {
        ...
        auto resp = MakeStringResponse(http::status::bad_request, text, http_version, keep_alive, ContentType::APP_JSON, true);
        return resp;
    }
    ...
    FileRequestResult BaseRequestHandler::Handle(const StringRequest& req) const {
        try {
            ...
            default:
                return MakeStringResponse(http::status::method_not_allowed, app::JsonMessage("invalidMethod"sv, "Invaled method"),
                                          req.version(), req.keep_alive(), ContentType::APP_JSON, true);
            }
        }
        catch (std::exception ex) {
            return MakeStringResponse(http::status::bad_request, app::JsonMessage("badRequest", "Server error: "s + ex.what()),
